...
**Glow rendering note:** Glow text on light backgrounds uses `#C8960C` (`glowOnLight`). Brand fill colour `#F5D000` is for fills only (bars, dot fills, circle fills, avatar borders, card backgrounds, canvas card). Avatar borders always use `#F5D000`, never `glowOnLight`. This applies in the app, emails, and any digital output where Glow appears as text. The `EnergyWord` component and `colorize()` helper use `glowOnLight` for Glow.
