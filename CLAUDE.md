...
104. ✅ Constellation: fluent arc moved inside the node — arc is now a dashed semicircle (`arcR = nodeR−5`, so 9/8/7 for nodeR 14/12/10) in the lower half of the node interior, coloured in the fluent energy colour. Rendered inside each node's `<g>` between the outer ring and initials. Arc shown only when `secondEnergyGap ≥ 4` (suppresses arc for members with flat profiles). Initials shifted to `cy+1`. Outer ring stroke bumped to 2.5px. Old standalone fluent arc render (below the node) removed.
