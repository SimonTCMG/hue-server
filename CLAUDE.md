...
- Team constellation view: SVG spatial layout — zone-based positioning guarantees every node sits inside its instinctive energy quadrant (hard constraint). Fluent (2nd) energy determines position within quadrant via pull table. X axis: Spark (right) vs Tend (left), Y axis: Flow (top) vs Glow (bottom). Quadrants: Spark top-right, Glow bottom-right, Flow top-left, Tend bottom-left. Radial gradient quadrant washes at ~0.22 opacity (Glow uses `#C8960C`). Thin axis lines (0.5px, 12% opacity). Corner-anchored labels (9px, 700 weight, 65% opacity, letter-spaced). No animation, no glow/halo — all nodes equal weight (cream fill, instinctive colour stroke). Dashed semicircular fluent arc below each node in fluent energy colour. Reach callouts in empty quadrants for absent instinctive energies with dashed leader lines (suppressed for 30+ members). Overlap nudging (40px min separation) with quadrant clamping after every pass. Weight bars below map: instinctive count (solid fill) + fluent count (lighter fill behind) per energy. Gap callout: one plain-English sentence about team spread. Legend: two items (instinctive circle + fluent dashed line). Responsive: node radius scales 14→12→10 by team size; arcs scale with nodes; callouts suppressed at 30+. Initials on nodes (disambiguated to first 4 chars of first name when two members share the same initials), full name + fluent energy on hover. List/table alternative for teams of 10+. No connection lines.
