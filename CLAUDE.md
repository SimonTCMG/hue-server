...
*Master brief for all Claude sessions. Last updated: 5 May 2026 (Daily email redesign v1: energy badges removed, content type label is now plain text, CTA pre-populates the companion input via `?prompt=` URL param, AI-generated subject lines using a 5-form typology, scenario lines (inline + full-email forms) on a 6-week schedule, share-this link added below the CTA, in-app companion referral that fires once per user after 30 days + 5 exchanges + 3 sessions. Spec: anyan-daily-email-redesign-v1.md (the brief uses "Anyan" as a potential future product name — assume Anyan = MyHue/Hue until the rename is confirmed).).*
...
### Working and live
- Full assessment conversation flow (AI-conducted, adaptive 8–18 exchanges, signal-density coverage gate, six scoring dimensions in system prompt). Includes always-all-four gradient progress line in the assessment screen header — fills as coverage points accumulate, drifts subtly while idle.
...
- Daily email: personalised AI-generated email sent at 8am UK to all subscribers/org members/beta users. Four-colour top bar (Spark/Glow/Tend/Flow stripes — represents the whole, not the individual), cream background. Energy badges removed (May 2026 redesign — see below). Content type label rendered as plain text only. Body copy energy names rendered in energy colours via `formatEmailCopy()`. CTA button ink black, links to `https://myhue.co?prompt={ENCODED_PROMPT}` so the companion input is pre-populated when the reader arrives. "Share this" plain-text link below the CTA opens a `mailto:` with the first sentence of the body and "Find your own at myhue.co". Footer: "MyHue · myhue.co — Your profile belongs to you." Email header: static spin logo + MyHue wordmark in Georgia serif. Animated logo not possible in email — static only. Content generated by Claude Sonnet 4.6 with full profile context. `stripMarkdown()` safety net strips residual formatting. Context block marked internal-only to prevent profile data leaking into email body.
- Daily email energy rotation: weekly focus mode cycle across all four energy positions — instinctive (Sun/Thu), fluent (Mon/Fri), intentional (Tue/Sat), developing (Wed). Each mode has a distinct content instruction: instinctive (what it makes possible), fluent (what it adds alongside instinctive — name the combination), intentional (precision when reached for — not a gap), developing (where practice has most visible impact — never a weakness). Rotation invisible to recipient — no energy name in body copy, no badges. Subject line may include energy name on intentional/developing days only.
- Two-facts technique: occasional overlay (~1 in 7 emails, fires every other Wednesday on developing day) — exactly two standalone sentences, both observable behaviours, no connective, no question, no follow-up. The reader closes the gap. Never on fluent days. Content type label stays as "A thought for today" on two-facts days. Subject line on two-facts days uses the two-facts subject form (slight tension, no resolution).
- Daily email subject typology (May 2026): five forms — `partial-observation` (common), `deliberately-incomplete` (rationed to ≤ once per fortnight per user, tracked via `last_subject_form`/`last_subject_form_at`), `question` (common), `scenario-hook` (common), `two-facts` (only on two-facts days). AI selects the form per email; server enforces the deliberately-incomplete fortnight gap and forces `two-facts` on two-facts days. Subjects must never reference the day of the week, never use the person's name, never imply the rotation is visible. Subject line is now AI-generated alongside the body (single API call returns labelled output: `SUBJECT_FORM`, `SUBJECT`, `CONTENT`, `COMPANION_PROMPT`, optional `SCENARIO_LINE`).
- Daily email companion prompt: AI generates a one-sentence conversation opener (`COMPANION_PROMPT` field) alongside the body. URL-encoded into the CTA so when the reader taps "Continue in Hue" the companion opens with that prompt pre-populated in the input field — never auto-submitted. The user sends, edits, or ignores. If the user is not logged in, the prompt is preserved through registration/login via `sessionStorage.hue_pending_prompt` and consumed on first companion mount. If a user is authenticated but has no profile yet, the prompt is silently cleared (graceful fallback — no error page).
- Daily email scenario lines (§5 of redesign): two forms, alternating, on a 6-week-minimum schedule per user. Inline form weaves a single italic line below the main content (`{{SCENARIO_LINE_BLOCK}}`); full-email form rebuilds the entire body around a scenario from approved territory (the conversation you've been putting off / a decision that keeps coming back / when you can see what's happening but can't find the words / what's draining you that you can't quite name / a room where you felt completely in your element — or completely out of it / the thing you nearly said). Server tracks `last_scenario_at` and `last_scenario_form` per user; never fires on a two-facts week. Scenario rules baked into AI prompt: never reference other users, no recency language, no labelling by energy, no instruction, plain universal language, stop just short of resolution.
- Daily email "Share this" link: plain-text underlined link below the CTA — never an icon row of social platforms, never a primary visual element. Opens `mailto:` with body pre-filled (first sentence of email + "Find your own at myhue.co"). User edits before sending. The Anyan/Hue link is always the last line. The share is private — no who-was-shared-to tracking, only the standard click event.
...
- Team constellation view: SVG spatial layout — zone-based positioning guarantees every node sits inside its instinctive energy quadrant (hard constraint). Fluent (2nd) energy determines position within quadrant via pull table. X axis: Spark (right) vs Tend (left), Y axis: Flow (top) vs Glow (bottom). Quadrants: Spark top-right, Glow bottom-right, Flow top-left, Tend bottom-left. Radial gradient quadrant washes at ~0.22 opacity (Glow uses `#C8960C`). Thin axis lines (0.5px, 12% opacity). Corner-anchored labels (9px, 700 weight, 65% opacity, letter-spaced). No animation, no glow/halo — all nodes equal weight (cream fill, instinctive colour stroke, 2.5px). Fluent arc is a dashed semicircle INSIDE the node (lower half, arcR = nodeR−5), in the fluent energy colour — shown when `bands[secondEnergy]` is "Naturally present" or "Intentionally present"; suppressed when "Developing". `second_energy` (individual profile's ranked[1] label) and `second_energy_gap` (pct gap to ranked[2], retained for analytics) are both stored in `team_energy_bands`. Members whose second energy is Developing (flat profile) show no arc. Initials sit above node centre (cy+1) to clear the arc. Deterministic jitter applied to nodes sharing an identical base position before overlap nudging, preventing horizontal clustering. Reach callouts in empty quadrants for absent instinctive energies with dashed leader lines — suppressed for 30+ members AND suppressed between diagonally opposite quadrants (spark↔tend, glow↔flow). Overlap nudging (40px min separation) with quadrant clamping after every pass. Weight bars below map: instinctive count (solid fill) + fluent count (lighter fill behind) per energy. Gap callout: one plain-English sentence about team spread. Legend: two items — item 1: plain circle (instinctive ring only); item 2: circle with both outer ring and inner dashed arc (fluent). Both grey. Responsive: node radius scales 14→12→10 by team size; callouts suppressed at 30+. Initials on nodes (disambiguated to first 4 chars of first name when two members share the same initials), full name + fluent energy on hover. List/table alternative for teams of 10+. No connection lines.
...
- Sticky header on all team-scoped views: `position: sticky; top: 0; zIndex: 100; background: #FDFAF4; borderBottom: 1px solid sand; padding: 20px 40px / 16px 20px`. SpinMark (38px, paused) + "Hue" wordmark left (Fraunces 20px 700 — matches Results page exactly), team name centred with ellipsis (Plus Jakarta Sans 600), NavIcons right: checkin + share + home for leads/admins; home only for members. `checkin` NavIcon type: circle + exclamation, active state when check-in open. Outer wrapper has no `overflow` property — `overflowX: hidden` breaks `position: sticky`. Appears on loading, error, reveal-gate, and main dashboard states.
...
- Readback: three slots. Slot 1 = template-based aggregate (top Q1 dimensions + Q2 landing distribution). Slot 2 = **null for TCMG beta** — observation library (~40 sentences) not yet written; `generateReadback()` returns null and `TeamCheckinReadback` must render a 2-slot layout with no placeholder copy. Slot 3 = keyed prompt from `CHECKIN_SLOT3_PROMPTS` dict (stretch/matched/celebratory/quiet/mixed) based on Q2 majority. Fortnightly parity: even ISO weeks are "on". Readback gated: closed + `response_count >= checkin_min_responses` + `dashboard_revealed` (leads bypass reveal gate). `TeamCheckinReadback` component renders between 32-dimension panel and constellation.
...
- Auto-sync: assessment completion pushes bands to all teams user belongs to. Team dashboard endpoint also auto-syncs missing bands on load (handles case where user completed assessment before being added to a team, and backfills `second_energy`/`second_energy_gap` for legacy records)

### Org admin — UI built
- Org admin screen at `/org/:orgId`: create teams, invite members by email (sends invite via MailerSend with data ownership message), view all members with status (Profile complete / Joined, not started / Invited), remove members, assign team leads. Member status reads from canonical `assessment_completed_at` on users table. Filter controls: filter by team (dropdown) and filter by status (All / Profile complete / Pending). Sticky header: SpinMark + "Hue" left, org name centred, home NavIcon right (no share — share is team-level). Back arrow removed; home NavIcon handles navigation. Org name heading remains in page body below header.
...
**Assessment format (v2.1, live 2 May 2026):** Conversational, AI-conducted, adaptive **8–18 exchanges**. Minimum floor **8 exchanges**. Coverage gate: five counters (valence ≥2, crossContext ≥2, recovery ≥2, updateMechanism ≥1, identityInSuccess ≥1) must all pass before completion JSON is emitted. **Density-aware increments:** counters increment by the number of distinct situations or contexts in a response, capped at +2 per counter per response — never by word count. A single rich response may legitimately satisfy multiple counters at once. Ceiling 18: JSON emitted regardless of coverage at exchange 18. Question bank: 7 original topics + Q9–Q16 (8 new questions, each tagged to a coverage counter). Exchanges 1–6 draw from the original bank; exchanges 7+ are coverage-driven (lowest counter first). Behavioural questions ("what did you do") not hypothetical ("what would you prefer"). **Coverage emission:** model appends a hidden `<!--coverage:{...}-->` line on every non-completion turn so the frontend can drive the progress line. **Visual indicator:** always-all-four-colours gradient line in the assessment screen sticky header (STARTED ──── COMPLETE), 14s idle drift, width grows with coverage points (8 total). See `hue-psychology-foundations-v1.md` for the theoretical basis (Mischel, Gosling). See `hue-assessment-v2-code-brief.md` for the full v2.1 spec.
...
**Team constellation — positioning (redesigned April 2026):** Zone-based placement replaces the ratio formula. Instinctive energy determines quadrant (hard constraint — no member may ever appear outside their instinctive quadrant). Fluent (2nd) energy determines position within the quadrant via a pull table: the node is placed 25% toward the shared boundary between the instinctive and fluent quadrants. If fluent = instinctive or the profile is flat, the node sits at quadrant centre. Overlap nudging (40px min separation, iterative with quadrant clamping after every pass) ensures no node crosses into the wrong quadrant. Axis mapping unchanged: X = Spark (right) vs Tend (left), Y = Flow (top) vs Glow (bottom). Quadrants: Spark top-right, Glow bottom-right, Flow top-left, Tend bottom-left. No connection lines. No animation. No glow/halo. Weight bars and gap callout below the map. Reach callouts for absent instinctive energies (suppressed for 30+ members).
...
117. ✅ Assessment v2.1 — signal-density gate (floor 12→8, density-aware counter increments capped at +2 per counter per response, multi-counter increments allowed); always-all-four-colours gradient progress line in the assessment screen header (`HueProgressLine` component, 14s idle drift, fills as coverage points accumulate); coverage emission via hidden `<!--coverage:{...}-->` line per turn parsed and stripped frontend-side; time estimates updated to "around 10 minutes" in consent conversation, home screen, and consent system prompt. See `hue-assessment-v2-code-brief.md` §4 and §5. (2 May 2026)
...
**Constellation redesign — completed 16 April 2026:**

87. ✅ Constellation full redesign — zone-based positioning (instinctive = quadrant, fluent = position within quadrant), radial gradient washes at 0.22 opacity, dashed fluent arcs below nodes, weight bars with instinctive/fluent counts, gap callout sentence, reach callouts for absent instinctive energies, two-item legend (instinctive circle + fluent dashed line), responsive node scaling (14→12→10 by team size). Replaces ratio formula, removes ambient float animation, removes node glow/halo. All nodes equal weight (cream fill, instinctive colour stroke).
...
89. ✅ Constellation overlap nudging — iterative collision resolution (40px min, 50 passes) with quadrant clamping after every pass
...
**Daily email redesign v1 — completed 5 May 2026** (spec: `anyan-daily-email-redesign-v1.md`; "Anyan" in spec means MyHue/Hue until rename is confirmed):

118. ✅ Energy badges removed: `{{ENERGY_NAME}}`, `{{ENERGY_COLOR}}`, `{{ENERGY_BADGE_TEXT}}`, `{{SECOND_ENERGY_NAME/COLOR/BADGE_TEXT}}` placeholders all removed from `hue-email-template.html`. Replaced with `{{CONTENT_TYPE_LABEL}}` (plain text only — no coloured element). The four-colour top bar remains and represents the whole, not the individual.
119. ✅ Companion-prompt CTA: CTA links to `https://myhue.co?prompt={ENCODED_PROMPT}`. AI generates a `COMPANION_PROMPT` field alongside the body — a one-sentence conversation opener URL-encoded into the link. On arrival, hue.html reads `?prompt=` from `URLSearchParams`, captures it to a `pendingPrompt` state slot and `sessionStorage.hue_pending_prompt`, then cleans the URL via `replaceState`. When the authenticated user has a profile, an effect navigates straight to `companion` with the prompt pre-populated. Unauthenticated visitors complete register/login first; the prompt persists via sessionStorage and is consumed on first companion mount. Falls back to default companion state if profile is missing — never an error page. `CompanionScreen` accepts `initialInput` + `onConsumeInitialInput` props.
120. ✅ Subject line typology (5 forms): `partial-observation`, `deliberately-incomplete`, `question`, `scenario-hook`, `two-facts`. Subject is now AI-generated alongside the body in a single Anthropic call returning labelled output (`SUBJECT_FORM` / `SUBJECT` / `CONTENT` / `COMPANION_PROMPT` / optional `SCENARIO_LINE`). `parseStructuredOutput()` and `firstSentence()` helpers in server.js. Server enforces: `two-facts` form on two-facts days, `deliberately-incomplete` rationed to ≤ once per fortnight (tracked in `users.last_subject_form` + `last_subject_form_at`), no day-of-week reference, no person's name, no implication that rotation is visible. Old hard-coded `${contentType} — your ${energy} energy — ${day}` subject removed.
121. ✅ Share button: plain-text underlined link below the CTA in the daily email — `mailto:?body={encoded share text}`. Share text is the first sentence of the body + "Find your own at myhue.co" (extracted programmatically from the AI body). Plain text only, never an icon row of social platforms. Tracking via standard email click event only — no recipient tracking. Trial and nurture emails are unaffected.
122. ✅ Scenario lines (§5): two forms, alternating, with a 6-week minimum gap per user. Inline form renders a single italic line below the main content via `{{SCENARIO_LINE_BLOCK}}`. Full-email form rebuilds the body instruction around a scenario drawn from approved territory (six entries: putting-off conversation, recurring decision, can-see-but-can't-say, draining-and-can't-name, in-or-out-of-element room, the thing nearly said). `SCENARIO_TERRITORY` const + `SIX_WEEKS_MS`. Scenario rules in AI prompt: no other-user reference, no recency, no energy label, no instruction, plain universal language, stop short of resolution. Never fires on a two-facts week. Server records form via `recordScenarioSent(userId, form)` after successful send (`users.last_scenario_at`, `users.last_scenario_form`).
123. ✅ In-app companion referral (§6): single-fire-ever prompt delivered IN-CONVERSATION via system addendum on `/api/chat`. Triggers (all required): user 30+ days since `trial_started_at` || `registered_at`, `companion_exchange_count >= 5`, distinct `companion_session_days` count >= 3, `referral_prompt_sent === 0`. `recordCompanionExchange(userId)` runs on every `/api/chat` call (increments count, dedupes today's date into the JSON-encoded `companion_session_days` array, capped at last 30 days). When eligibility hits and the current session has ≥3 user messages, server appends an addendum telling the AI it MAY ask once at a natural break: "You've been using this for a month. Has it been useful enough to mention to anyone?" — and on yes responds: "The best way is to send them to myhue.co — they'll find their own way in." Marked sent on injection so it never fires again, even if the AI chooses to skip the moment. No incentive, no link, no mechanism, no referral programme reference. `isReferralEligible(userId)` + `markReferralPromptSent(userId)` helpers in db.js.
124. ✅ DB migrations (idempotent ALTER TABLE): `users.last_scenario_at`, `last_scenario_form`, `last_subject_form`, `last_subject_form_at`, `referral_prompt_sent`, `companion_exchange_count`, `companion_session_days`. New helpers exported from `db.js`: `recordScenarioSent`, `recordSubjectForm`, `recordCompanionExchange`, `markReferralPromptSent`, `isReferralEligible`.
...
107. ✅ Sticky header on all team-scoped views — SpinMark + "Hue" wordmark left, team name centred, check-in/share/home NavIcons right (leads); home only (members). Matches Results page pattern exactly. Appears on loading, error, reveal-gate, and main states.
115. ✅ Header consistency pass — Results page pattern applied to team dashboard, org admin, and shared team pages. `checkin` NavIcon type added (circle + exclamation). `overflowX: hidden` removed from outer wrappers (was breaking `position: sticky`). OrgAdminScreen: sticky header replaces Back arrow + inline h1. SharedTeamScreen: SpinMark + "Hue" left, team name + "Read only" badge centred, myhue.co link right (no home — unauthenticated view).
...
109. ✅ Weekly check-in system — Pipeline 3 (team state, anonymous). `team_checkins` + `team_checkin_responses` tables in db.js (all CRUD helpers confirmed live). `checkin-dimensions.json` config (32 dims, 12 defaultVisible). API routes live: GET current, POST ad-hoc, POST respond, GET readback, GET latest, PUT notification-email. Three cron jobs: Friday 15:00 open, Sunday 18:00 reminder, Monday 09:00 close. `resolveTeamEmail()` + `getISOWeek()` helpers. `generateReadback()` — Slot 1 template aggregate, Slot 2 null (observation library not yet written, frontend must render 2-slot layout), Slot 3 from `CHECKIN_SLOT3_PROMPTS` dict (stretch/matched/celebratory/quiet/mixed). Fortnightly parity: even ISO weeks are "on". Email templates inline via `checkinEmailHtml` (reuses `trialEmailHtml`). `db.js`: `getTeamMembersForNotification()` + `getTeamsEligibleForCheckin()` added.
110. ✅ Check-in frontend — `TeamCheckinFlow` full-screen overlay (4 questions, dimension chips grouped by energy, Q2 radio, Q3/Q4 optional textarea). `TeamCheckinReadback` 3-slot card on dashboard (between 32-dimension panel and constellation). Check-in notification banner. Ad-hoc trigger panel (leads only). Frontend must handle null Slot 2 gracefully — render 1 and 3 only, no placeholder copy.
...
112. ✅ `/api/team/:teamId/checkin/latest` endpoint — returns most recent closed check-in readback, respects dashboard reveal gate and anonymity threshold. Leads may view readback before reveal; members see gated response until `dashboard_revealed = true`.
113. ⬜ Slot 2 check-in observation library — ~40 curated sentences to write with Simon. `generateReadback()` currently returns `slot2: null`; frontend renders 1 and 3 only. Separate piece of work — see `hue-observations-v1.md` as the source when Simon is ready.
...
102. ✅ Constellation: horizontal row clustering fixed — `jitterCollocated()` applies deterministic small offsets (golden-angle-ish spread, ~3px) to nodes sharing an identical base position before `resolveOverlaps` runs. Without this, nodes with the same instinctive+fluent combination (e.g. three Tend-Glow members) all start at dist=0 and the nudge never fires, producing a horizontal row.
103. ✅ Constellation: fluent energy source corrected — `second_energy TEXT` and `second_energy_gap INTEGER` columns added to `team_energy_bands` (idempotent ALTER TABLE migration). `calculateSecondEnergy(scores, dominant)` added to `db.js`. All six band-upsert sites in `server.js` now store the individual profile's ranked[1] energy and the pct gap to ranked[2]. `/api/team/:teamId` member payload includes `secondEnergy` + `secondEnergyGap`. Auto-sync widens to backfill legacy records with null `second_energy` on first dashboard load.
104. ✅ Constellation: fluent arc moved inside the node — arc is now a dashed semicircle (`arcR = nodeR−5`, so 9/8/7 for nodeR 14/12/10) in the lower half of the node interior, coloured in the fluent energy colour. Rendered inside each node's `<g>` between the outer ring and initials. Arc shown when `bands[secondEnergy]` is "Naturally present" or "Intentionally present" — suppressed when "Developing" (flat profile, e.g. Nicky). A 4pt gap threshold was tried first but proved too tight for real profiles where scores bunch closely (e.g. Cathy's Flow at ~24% with gap=2pt). `secondEnergyGap` retained on the record for future analytics. Initials shifted to `cy+1`. Outer ring stroke 2.5px. Old standalone arc render removed.
105. ✅ Constellation: diagonal leader line suppression — reach callout leader lines to fluent nodes in diagonally opposite quadrants are now suppressed. `DIAGONAL_OPPOSITE = { spark: "tend", tend: "spark", glow: "flow", flow: "glow" }`. A leader from the Spark callout (top-right) to a Tend-instinctive member (bottom-left) would slash the map diagonally — now filtered. Leader line endpoint updated to node edge (was arc bottom, now `y − nodeR − 2`).
106. ✅ Constellation legend updated: item 1 (instinctive) unchanged — plain grey circle. Item 2 (fluent) now shows a grey circle with both outer ring and inner dashed arc, matching the actual node anatomy. Old fluent legend (horizontal dashed line) removed.
